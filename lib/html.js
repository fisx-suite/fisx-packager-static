...
function parsePackScript(file, context, opts) {
    ...
    var getPlaceholder = function () {
        ...
        // 如果指定了异步入口模块，则在前面插入同步加载的脚本
        var placeholder = opts.scriptPlaceholder;
        hasScriptPlaceholder = true;

        // 在异步入口模块前插入 requrie.config 脚本
        if (!hasRequireConfigPlaceholder) {
            placeholder += ('\n' + opts.resourceConfigPlaceholder);
        ...
        function (all, comment, script, attrs, body) {
            ...
            if (!body.trim() && SRC_HREF_ATTR_REGEXP.test(attrs)) {
                ...
                if (LOADER_ATTR_REGEXP.test(attrs)
                    || (~opts.loaderScripts.indexOf(
                        scriptFile ? scriptFile.basename : _.ext(src).basename)
                    )) {
                ...
                // 对于内联脚本，如果指定了异步入口模块，则在前面插入脚本占位符
    ...
    if (packScriptFiles.length && !hasScriptPlaceholder) {
...
function parsePackStyle(file, context, opts) {
    ...
    content = content.replace(LINK_STYLE_ELEM_REGEXP,
        function (all, comment, link, linkAttr) {
            ...
            if (link && REL_ATTR_REGEXP.test(linkAttr) && SRC_HREF_ATTR_REGEXP.test(linkAttr)) {
                var href = RegExp.$2;
                var styleFile = context.getFileByUrl(href, file);
                if (styleFile && opts.packCss) {
                    linkAttr = linkAttr.replace(SRC_HREF_ATTR_REGEXP, '').replace(/\s+$/, '');
                    packStyleFiles.push({
                        file: styleFile,
                        raw: all,
                        attrs: linkAttr
    ...
    if (packStyleFiles.length && !~content.indexOf(opts.stylePlaceholder)) {
