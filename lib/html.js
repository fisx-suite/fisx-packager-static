        ...
        function (all, comment, script, attrs, body) {
            ...
            if (!body.trim() && SRC_HREF_ATTR_REGEXP.test(attrs)) {
                ...
                if (ENTRY_ATTR_REGEXP.test(attrs)
                    || _.extractAsyncModuleIds(body).length) {
...
module.exports = exports = {
     ...
     * @return {string}
     */
    getRequireConfigScript: function (config) {
        return 'require.config(' + JSON.stringify(config, null, 2) + ');';
    },
    ...
    createRequireConfigScript: function (config) {
        return '<script>\n' + this.getRequireConfigScript(config) + '\n</script>';
    },
